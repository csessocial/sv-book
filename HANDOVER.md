...
- **수집 소스 3곳**: 교보문고 · 국회도서관(금주의 신간, 최근 3개월) · 네이버 도서
...
기본 수집 소스는 `generate.py` 하단 `sources = ["kyobo", "nanet", "naver"]` 에서 조정합니다.
...
| 키 | 위치 | 용도 |
|----|------|------|
| 네이버 도서 API (`NAVER_CLIENT_ID`, `SECRET`) | `config.py` | 네이버 신간·책 소개 검색 |
...
1. **네이버 도서 수집 0건** — 최근 네이버 도서 API가 결과를 반환하지 않습니다(키 만료 또는 API 종료 가능성).
   교보·국회도서관으로 신간은 충분히 나오지만, 복구하려면 **네이버 개발자센터(developers.naver.com)**에서
   앱/키 상태와 '책 검색' API 제공 여부를 확인하세요.
2. **AI 요약·저자 소개 비활성** — `ANTHROPIC_API_KEY` 미등록 상태. 7장대로 등록하면 켜집니다.
