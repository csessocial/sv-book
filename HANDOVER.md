...
| `featured_history.json` | 이미 "이달의 추천"에 나온 책 이력 (자동 물갈이용, 자동 생성물) |
| `.github/workflows/update.yml` | 자동 실행 스케줄 정의 |

---

## 4. 사이트 업데이트(수집·배포) 방법 — 다음 담당자 필독 ⭐

수집을 한 번 돌리면 **수집 → 필터·점수 → HTML 생성 → main에 자동 커밋 → GitHub Pages 배포**까지
전 과정이 자동으로 진행됩니다. 약 **6~13분** 후 사이트에 반영됩니다.

### 정기 자동 실행
- 매주 **월요일 오전 10시(KST)** 자동 수집·배포 (`update.yml`의 cron). 담당자가 아무것도 안 해도 돌아감.

### 수동 실행 (셋 중 편한 것)
1. **GitHub → Actions 탭 → "SV Book 자동 수집" → Run workflow** 버튼 ← 가장 간단, 권장
2. **사이트의 "지금 실행" 버튼** — 본인 GitHub Personal Access Token 입력 시 시작
   (토큰 권한: 저장소 `workflow` 실행 권한 필요)
3. **로컬 실행**: `python generate.py` (아래 "로컬에서 실행하기" 참고)

### ⚠️ 딱 하나 지킬 규칙: **한 번에 하나만 실행**
- 이미 수집이 돌고 있을 때 또 실행하거나 버튼을 두 번 누르면, 나중 실행이
  **push 충돌(non-fast-forward)로 실패**합니다. (사이트가 깨지진 않음 — 데이터는 누적이라 안전)
- 실행 후에는 **Actions 탭에서 초록 ✅ 로 끝나는 것만 확인**하고 기다리면 됩니다.

### 반영 확인
- 브라우저 탭 제목의 날짜(`SV Book — YYYY.MM.DD HH:MM`)가 방금 실행 시각이면 최신 버전.
- 안 바뀌면 **강력 새로고침(Ctrl+Shift+R)** — GitHub Pages는 최대 10분 캐싱.
...
### 💡 "이달의 추천(캐러셀 1~5위)"은 자동으로 물갈이됩니다
- 추천 5권은 `generate.py`의 `select_featured()`가 관련도 5점 후보 중에서 자동 선정합니다.
- **한 번 추천된 책은 `featured_history.json`에 기록되어 다음 수집 때 제외**됩니다 → 매번 새 5권으로 교체.
- 같은 책의 소스별 중복(예: 네이버본·국회도서관본)도 자동 제거됩니다.
- **담당자가 따로 할 일은 없습니다** — 그냥 수집만 돌리면 알아서 물갈이됩니다.
- 추천을 처음부터 다시 돌리고 싶으면 `featured_history.json`을 비우거나(`[]`) 삭제하면 됩니다.
