...
/* ── 히어로 ── */
.hero{{margin-top:60px;background:linear-gradient(135deg,#1a1f36 0%,#0f1d5e 40%,#1a1051 70%,#0d1117 100%);min-height:560px;display:grid;grid-template-columns:1fr 1fr;position:relative;overflow:hidden}}
...
.c-book-main img{{width:176px;height:264px;border-radius:10px;display:block;object-fit:cover;transition:transform .35s;border:1px solid rgba(255,255,255,.1)}}
...
/* 카드 */
.book-card{{background:white;border-radius:var(--radius);border:1px solid var(--border);display:flex;flex-direction:column;opacity:0;transform:translateY(24px);transition:opacity .45s,transform .45s,box-shadow .25s,border-color .25s}}
...
.cover-wrap{{position:relative;overflow:hidden;border-radius:var(--radius) var(--radius) 0 0;background:#eef0fb}}
.cover-wrap img{{width:100%;aspect-ratio:3/4;object-fit:cover;display:block;transition:transform .45s ease}}
.book-card:hover .cover-wrap img{{transform:scale(1.06)}}
.cover-overlay{{position:absolute;inset:0;background:linear-gradient(to top,rgba(26,31,54,.65) 0%,transparent 55%);opacity:0;transition:opacity .3s}}
.book-card:hover .cover-overlay{{opacity:1}}
.overlay-link{{position:absolute;bottom:12px;left:50%;transform:translateX(-50%) translateY(6px);opacity:0;transition:all .25s;white-space:nowrap;background:white;color:var(--accent);font-size:.71rem;font-weight:700;padding:6px 16px;border-radius:20px;text-decoration:none}}
.book-card:hover .overlay-link{{opacity:1;transform:translateX(-50%) translateY(0)}}
.badge-new{{position:absolute;top:8px;left:8px;background:linear-gradient(135deg,#ef4444,#f97316);color:white;font-size:.58rem;font-weight:700;padding:3px 8px;border-radius:4px;letter-spacing:.8px}}
.cat-pill{{position:absolute;top:8px;right:8px;font-size:.59rem;font-weight:700;padding:3px 9px;border-radius:20px;letter-spacing:.3px}}
...
.src-dot{{position:absolute;bottom:8px;right:8px;font-size:.57rem;font-weight:600;padding:2px 7px;border-radius:4px;background:rgba(255,255,255,.9);color:var(--ink3)}}

.book-body{{padding:13px 15px 14px;flex:1;display:flex;flex-direction:column;gap:7px}}
.book-title{{font-size:.87rem;font-weight:700;color:var(--ink);line-height:1.42;letter-spacing:-.15px}}
.book-title a{{color:inherit;text-decoration:none}}
.book-title a:hover{{color:var(--accent)}}
.book-meta{{font-size:.68rem;color:var(--ink4);display:flex;gap:4px;flex-wrap:wrap;align-items:center;font-weight:300}}
.meta-sep{{opacity:.3}}
.book-desc{{font-size:.74rem;color:var(--ink3);line-height:1.72;flex:1;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden;font-weight:300}}
.book-desc.expanded{{display:block}}
.book-footer{{display:flex;align-items:center;justify-content:space-between;padding-top:9px;border-top:1px solid var(--border);margin-top:3px}}
.score-badge{{font-size:.65rem;font-weight:700;color:var(--accent);background:var(--accent-light);padding:2px 9px;border-radius:20px}}
.toggle-btn{{font-size:.69rem;color:var(--accent);cursor:pointer;font-weight:600;background:none;border:none;font-family:var(--sans);padding:0}}
