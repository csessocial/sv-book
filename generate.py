...
/* ── 미니 테스트 (플로팅) ── */
.quiz-fab{{position:fixed;bottom:28px;right:28px;z-index:400;width:56px;height:56px;border-radius:50%;border:none;background:linear-gradient(135deg,#4361ee,#7c3aed);color:white;font-size:1.5rem;cursor:pointer;box-shadow:0 6px 24px rgba(67,97,238,.4);display:flex;align-items:center;justify-content:center;transition:all .25s;animation:fabBounce 2s ease-in-out infinite 3s}}
.quiz-fab:hover{{transform:scale(1.08);box-shadow:0 10px 32px rgba(67,97,238,.5)}}
...
<!-- 미니 테스트 (플로팅) -->
<button class="quiz-fab" id="quizFab" onclick="openQuiz()" title="도서 추천 테스트">📚</button>
<div class="quiz-fab-tip" id="quizTip" style="display:none">나에게 맞는 책 찾기!</div>
...
function openQuiz(){{
  ...
  document.getElementById('quizTip').style.display='none';
}}
function closeQuiz(){{
...
// 3초 후 툴팁 표시, 6초 후 사라짐
setTimeout(()=>{{document.getElementById('quizTip').style.display='block';}},3000);
setTimeout(()=>{{document.getElementById('quizTip').style.display='none';}},7000);
